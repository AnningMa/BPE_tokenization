...
class NaiveBPE:
    ...
    def _init_splits(self):
        self._splits = {
            w: list(w) for w in self._word_freqs
        }  # O(W·L) where W is the number of unique words and L is the average word length
    ...
    def train(self, vocab_size, *, corpus=None, word_freqs=None):
        ...
        ):  # 循环执行M次，每次扫描所有为一次 --> O(M·W·L)
            pair_freqs = self._compute_pair_freqs()
            if not pair_freqs:
                break

            best = max(
                pair_freqs, key=pair_freqs.get
            )  # O(p) where p is the number of unique pairs
            a, b = best
            merged = a + b
    ...
    def tokenize(
        self, text
    ):  # O(M·L·T） where M is the number of merges, L is the average word length, and T is the number of tokens in the input text. In practice, this can be quite slow for large vocabularies and long texts.
